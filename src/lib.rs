...
mod component {
    ...
    #[act_tool(description = "Compute HMAC signature for message authentication", read_only)]
    fn hmac(
    ...
    fn jwt_decode(#[doc = "JWT token string"] token: String) -> ActResult<String> {
        ...
        if parts.len() != 3 {
            return Err(ActError::invalid_args("Invalid JWT: expected 3 dot-separated parts"));
        }
